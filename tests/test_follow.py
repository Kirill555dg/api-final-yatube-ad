...
class TestFollowAPI:
    ...
    def test_follow_not_auth(self, client, follow_1, follow_2):
        assert_msg = f"Проверьте, что GET-запрос неавторизованного пользователя к `{self.url}` возвращает ответ со статусом 401."
        try:
    ...
    def test_follow_create(
        ...
        assert test_data.get("following") == data["following"], (
            msg_error.format(
                additional_msg=(
                    "Сейчас ключ `following` отстутствует или содержит некорректное значение."
                )
            )
        )
