...
class TestFollowAPI:
    ...
    def test_follow_not_auth(self, client, follow_1, follow_2):
        assert_msg = (
            f"Проверьте, что GET-запрос неавторизованного пользователя к `{self.url}` возвращает ответ со статусом 401."
        )
        try:
    ...
    def test_follow_get(self, user_client, user, follow_1, follow_2, follow_3):
        ...
        assert response.status_code == HTTPStatus.OK, (
            f"Проверьте, что GET-запрос авторизованного пользователя к `{self.url}` возвращает ответ со статусом 200."
        )

        test_data = response.json()
        assert isinstance(test_data, list), (
            f"Проверьте, что GET-запрос авторизованного пользователя к `{self.url}` возвращает данные в виде списка."
        )

        num_of_follows = Follow.objects.filter(following__username=user.username).count()
    ...
    def test_follow_create(self, user_client, follow_2, follow_3, user, user_2, another_user):
        ...
        assert type(test_data) is dict, msg_error.format(additional_msg="")
        assert test_data.get("user") == user.username, msg_error.format(
            additional_msg=("Сейчас ключ `user` отстутствует или содержит некорректное значение.")
        )
        assert test_data.get("following") == data["following"], msg_error.format(
            additional_msg=("Сейчас ключ `following` отстутствует или содержит некорректное значение.")
        )
    ...
    def test_follow_search_filter(
        self, user_client, follow_1, follow_2, follow_3, follow_4, follow_5, user, user_2, another_user
    ):
