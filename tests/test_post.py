...
class TestPostAPI:
    ...
    def test_posts_auth_get(self, user_client, post, another_post):
        ...
        assert response.status_code == HTTPStatus.OK, (
            f"Проверьте, что GET-запрос авторизованного пользователя к `{self.post_list_url}` возвращает статус 200."
        )

        test_data = response.json()
        assert isinstance(test_data, list), (
            f"Проверьте, что GET-запрос авторизованного пользователя к `{self.post_list_url}` возвращает список."
        )
    ...
    def test_post_unauth_create(self, client, user, another_user):
        ...
        assert posts_conut == Post.objects.count(), (
            f"Проверьте, что POST-запрос неавторизованного пользователя к `{self.post_list_url}` не создаёт новый пост."
        )
